...
zed_sdk_path = r"C:\Users\ML-2\Documents\GitHub\zed-sdk"
sys.path.append(zed_sdk_path)
...
class GLViewer:
            ...
            if self.render_object(_objs.object_list[i]):
                ...
                if bounding_box.any():
                    ...
                    if _objs.object_list[i].tracking_state != sl.OBJECT_TRACKING_STATE.OK:
                        ...
                        self.create_id_rendering(pos, color_id, _objs.object_list[i], _objs.timestamp.get_microseconds()/1000000.0)
    ...
    def create_id_rendering(self, _center, _clr, _obj, _time):
        tmp = ObjectClassName()
        tmp.name = "ID: " + str(_obj.id)
        tmp.velocity = "Velocity: " + str(round(calculate_magnitude(_obj.velocity), 2))
        tmp.positions = f"Position: [{round(_obj.position[0], 2)}, {round(-float(_obj.position[2]), 2)}, {round(_obj.position[1], 2)}]"
        tmp.displacement = "Displacement: " + str(round(calculate_magnitude([_obj.position[0], -float(_obj.position[2]), _obj.position[1]]), 2))
        tmp.timestamp = f"Time: {_time}"
        tmp.color = _clr
